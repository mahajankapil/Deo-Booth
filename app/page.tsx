...
export default function PhotoBoothApp() {
  ...
  // host status: false by default, switches to true in createRoom()
  ...
  // Cross-tab signaling using localStorage
  const sendSignalingMessage = (message: Omit<SignalingMessage, "senderId" | "timestamp">) => {
    ...
    try {
      const storageKey = `photobooth_${message.roomId}`
      let existingMessages: SignalingMessage[] = []
      ...
      try {
        ...
        console.warn("Error parsing existing messages, starting fresh:", parseError)
        existingMessages = []
      }

      existingMessages.push(fullMessage)

      // Keep only last 50 messages to prevent localStorage from growing too large
      ...
      if (!verification) {
        throw new Error("Failed to store message in localStorage")
      }

      console.log(`Sent ${message.type} message for room ${message.roomId}:`, fullMessage)

      // Force a storage event for same-tab detection with a small delay
      setTimeout(() => {
  ...
  const processSignalingMessages = useCallback(async () => {
    ...
    try {
      const messages: SignalingMessage[] = JSON.parse(localStorage.getItem(`photobooth_${roomId}`) || "[]")

      // Process only new messages from other users
      const newMessages = messages.filter(
        (msg) => msg.senderId !== userId && msg.timestamp > lastProcessedTimestamp.current,
      )
  ...
  }, [roomId, peerConnection, userId, isHost, isConnected])

  // Listen for localStorage changes (cross-tab communication)
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === `photobooth_${roomId}`) {
    ...
    // Also poll for messages in case storage events don't fire
    const interval = setInterval(processSignalingMessages, 1000)

    return () => {
      window.removeEventListener("storage", handleStorageChange)
      clearInterval(interval)
    }
  }, [processSignalingMessages, roomId])
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      localStorage.removeItem(`photobooth_${newRoomId}`)

      // Create room message with more data
      const roomMessage: SignalingMessage = {
        type: "room-created",
        data: {
          hostId: userId,
          created: Date.now(),
      ...
      // Store the room message directly
      localStorage.setItem(`photobooth_${newRoomId}`, JSON.stringify([roomMessage]))

      // Verify the room was created
      const verification = localStorage.getItem(`photobooth_${newRoomId}`)
      if (!verification) {
        throw new Error("Failed to create room in localStorage")
      }

      console.log(`Room ${newRoomId} created successfully:`, verification)
  ...
  const joinRoom = async (id: string) => {
    ...
    try {
      // Try multiple times to find the room with longer delays
      let roomExists = false
      let attempts = 0
      const maxAttempts = 15 // Increased attempts
      const baseDelay = 1000 // Increased base delay to 1 second

      while (!roomExists && attempts < maxAttempts) {
        attempts++
        setConnectionStatus(`Looking for room... (${attempts}/${maxAttempts})`)

        try {
          const storageData = localStorage.getItem(`photobooth_${targetRoomId}`)
          console.log(`Attempt ${attempts}: Storage data for ${targetRoomId}:`, storageData)

          if (storageData) {
            const existingMessages: SignalingMessage[] = JSON.parse(storageData)
            roomExists = existingMessages.some((msg: SignalingMessage) => {
              const isRoomCreated = msg.type === "room-created"
              const isRecent = Date.now() - msg.timestamp < 300000 // 5 minutes
              console.log(`Message check: type=${msg.type}, isRecent=${isRecent}, timestamp=${msg.timestamp}`)
              return isRoomCreated && isRecent
            })

            if (roomExists) {
              console.log(`Room ${targetRoomId} found after ${attempts} attempts`)
              break
            }
          }
        } catch (parseError) {
          console.error(`Error parsing storage data on attempt ${attempts}:`, parseError)
        }

        if (!roomExists && attempts < maxAttempts) {
      ...
      if (!roomExists) {
        ...
        console.error(`Room ${targetRoomId} not found after ${maxAttempts} attempts`)
        alert(
  ...
  const endCall = () => {
    ...
    // Clear signaling data for this room
    if (roomId) {
      localStorage.removeItem(`photobooth_${roomId}`)
    }
  }
